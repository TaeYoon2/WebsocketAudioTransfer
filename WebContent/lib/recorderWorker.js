...
this.onmessage = function(e){
  switch(e.data.command){
    ...
    case 'export16kMono':
      ...
      break;
    case 'export16kMonoSync':
      export16kMonoSync(e.data.type, e.data.buffer);
      break;
    case 'getBuffer':
      getBuffer();
      break;
...
function record(inputBuffer){
  ...
  fullBuffers.push(inputBuffer[0]);
  recLength += inputBuffer[0].length;
  fullLength += inputBuffer[0].length;
...
function exportWAV(type){
  ...
  var buffer = mergeBuffers(fullBuffers, fullLength);
...
function export16kMonoSync(type, data){
  var samples = resampler.resampler(data[0]);
  var dataview = encodeRAW(samples);
  var audioBlob = new Blob([dataview], { type: type });
  this.postMessage(audioBlob);
}
...
function getFullbuffer() {
	var buffers = [];
	buffers.push( mergeBuffers(fullBuffers, fullLength) );
	this.postMessage(buffers);
}
